...
export const spc668Controller = async (request: FastifyRequest<{ Body: { document: string; typeDocument: "CPF" | "CNPJ"; insumos: number[] } }>, reply: FastifyReply) => {
    const { document, typeDocument, insumos } = request.body;

  const spcUseCases = new SPCUseCases();

  try {
    const spc = await spcUseCases.spc668(document, typeDocument, insumos);
