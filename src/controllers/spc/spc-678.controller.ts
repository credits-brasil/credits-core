...
export const spc678Controller = async (request: FastifyRequest<{ Body: { document: string; typeDocument: "CPF" | "CNPJ"; insumos: number[] } }>, reply: FastifyReply) => {
    const { document, typeDocument, insumos } = request.body;

  const spcUseCases = new SPCUseCases();

  try {
    const spc = await spcUseCases.spc678(document, typeDocument, insumos);
